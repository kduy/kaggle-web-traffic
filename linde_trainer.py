...
from linde_hparams import build_from_set, build_hparams
from linde_feeder import VarFeeder
from linde_input_pipe import InputPipe, ModelMode, Splitter,FakeSplitter, page_features
from linde_model import Model
import argparse
...
def train(name, hparams, multi_gpu=False, n_models=1, train_completeness_threshold=0.01,
          seed=None, logdir='data_linde/logs', max_epoch=100, patience=2, train_sampling=1.0,
    ...
    with tf.device("/cpu:0"):
        inp = VarFeeder.read_vars("data_linde/vars")
        # if side_split:
    ...
    if save_best_model or save_from_step:
        saver_path = f'data_linde/cpt/{name}'
            ...
            for _ in tqr:
                ...
                if step % eval_every_step == 0:
                    ...
                    if save_best_model and epoch > 0 and eval_smape.last < best_smape:
                        ...
                        saver.save(sess, f'data_linde/cpt/{name}/cpt', global_step=step)
                    if save_from_step and step >= save_from_step:
                        saver.save(sess, f'data_linde/cpt/{name}/cpt', global_step=step)

                    if avg_sgd and ema_eval_stages:
                        ema_saver.save(sess, 'data_linde/cpt_tmp/ema',  write_meta_graph=False)
                        # restore ema-backed vars
                        ema_loader.restore(sess, 'data_linde/cpt_tmp/ema')

                        trainer.eval_step(sess, epoch, step, eval_batches, stages=ema_eval_stages)
                        # restore normal vars
                        ema_saver.restore(sess, 'data_linde/cpt_tmp/ema')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--logdir', default='data_linde/logs', help="Directory for summary logs")
    ...
    # preds = predict('data_linde/cpt/fair_365-15428', 380, hparams, verbose=True, back_offset=60, n_models=3)
    # print(preds)
