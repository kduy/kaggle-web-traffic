...
import linde_extractor as extractor
from feeder import VarFeeder
import numba
from typing import Tuple, Dict, Collection, List

root = '/mnt/md0/hackathon/kaggle-web-traffic/data_linde/'
def read_cached(name="linde_all") -> pd.DataFrame:
    """
...
def lag_indexes(begin, end) -> List[pd.Series]:
    ...
    """
    dr = pd.date_range(begin, end,  freq='H')
...
def run():
    ...
    features_days = pd.date_range(data_start, features_end,  freq='H')
    #dow = normalize(features_days.dayofweek.values)
    week_period = 7*24 / (2 * np.pi)
